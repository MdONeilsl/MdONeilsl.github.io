...
import { merge_normal_array, scale_normal_array, normal_from_height_map_array, mix_channels_array } from "./image.js";
...
export const normal_from_height_map = (src_data, src_width, src_height, options) =>
    execute_worker(
        "normal_map_ww", "norm_height",
        {
            src: { data: src_data.buffer, width: src_width, height: src_height },
            options
        },
        () => {
            console.log(`fallback`);
            return normal_from_height_map_array(src_data, src_width, src_height, options);
        },
        [src_data.buffer]
    );

export const mix_channels_map = (inputs, width, height, config) => {
    inputs = inputs.map(input => input.buffer);
    return execute_worker(
        ...
        () => {
            console.log(`fallback`);
            return mix_channels_array(inputs, width, height, config);
        },
        inputs
    );
}
