...
export const has_transparency = image_data => {
    ...
    let step = 4;
    if (image_data.length > 16384) step = 8;
    else if (image_data.length > 262144) step = 16;
    else if (image_data.length > 1048576) step = 32;

    for (let i = 3; i < image_data.length; i += step) {
        if (image_data[i] < 254) {
            return true;
        }
    }
...
export const next_step_size = (current, target) => {
    if (current > target) {
        const half = current >> 1;
        return Math.max(target, half);
    } else {
        const doubled = current << 1;
        return Math.min(target, doubled);
    }
};
