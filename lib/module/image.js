...
import { fast_clamp } from "./color.js";
...
// Unified Channel Mixing Protocol for PBR Textures

/**
 * Protocol Description:
 * This protocol defines a general channel mixer using linear combinations.
 * Configuration:
 * - constants: array[4] of numbers (R, G, B, A) - base values (JS: 0-255, shaders: 0-1).
 * - terms: array[4] of array of {input: int (0-based index), channel: int (0=R,1=G,2=B,3=A), coeff: number}
 * For each pixel and each output channel ch (0-3):
 *   val = constants[ch] + sum(terms[ch]): coeff * inputs[input].pixel[channel]
 * Output clamped to 0-255 (JS) or 0-1 (shaders).
 * Inputs: array of images/textures, all same size.
 * Supports all pack/unpack operations bidirectionally via configs.
 * Examples:
 * Pack gray from RGB:
 *   inputs: [rgb_image]
 *   constants: [0,0,0,255] (JS) or [0,0,0,1] (shaders)
 *   terms: [[{input:0,channel:0,coeff:1/3},{input:0,channel:1,coeff:1/3},{input:0,channel:2,coeff:1/3}],
 *           same for 1, same for 2, [] for 3]
 * Unpack RGB from gray:
 *   inputs: [gray_image]
 *   constants: [0,0,0,255] or [0,0,0,1]
 *   terms: [[{input:0,channel:0,coeff:1}], same for 1&2 with channel:0, []]
 * Pack color (albedo RGB + trans gray to RGBA):
 *   inputs: [albedo_image, trans_image]
 *   constants: [0,0,0,0]
 *   terms: [[{input:0,channel:0,coeff:1}], [{input:0,channel:1,coeff:1}], [{input:0,channel:2,coeff:1}], [{input:1,channel:0,coeff:1}]]
 * Unpack albedo (RGBA to RGB):
 *   inputs: [color_image]
 *   constants: [0,0,0,255] or [0,0,0,1]
 *   terms: [[{input:0,channel:0,coeff:1}], [{input:0,channel:1,coeff:1}], [{input:0,channel:2,coeff:1}], []]
 * Unpack transparency (RGBA to gray):
 *   inputs: [color_image]
 *   constants: [0,0,0,255] or [0,0,0,1]
 *   terms: [[{input:0,channel:3,coeff:1}], same for 1&2, []]
 * Pack ORM (occ gray to R, rough gray to G, met gray to B):
 *   inputs: [occ_image, rough_image, met_image]
 *   constants: [0,0,0,255] or [0,0,0,1]
 *   terms: [[{input:0,channel:0,coeff:1}], [{input:1,channel:0,coeff:1}], [{input:2,channel:0,coeff:1}], []]
 * Unpack occlusion (ORM to gray):
 *   inputs: [orm_image]
 *   constants: [0,0,0,255] or [0,0,0,1]
 *   terms: [[{input:0,channel:0,coeff:1}], same for 1&2, []]
 * (Similar for roughness: channel:1, metalness: channel:2)
 */

/**
 * Mixes channels from inputs based on config.
 * @param {Uint8ClampedArray[]} inputs - Input image data arrays (RGBA format, same dimensions).
 * @param {Number} width - width of the images.
 * @param {Number} height - height of the images.
 * @param {Object} config - {constants: number[4], terms: Array[4] of {input: number, channel: number, coeff: number}[]}
 * @returns {Uint8ClampedArray} Output image data as Uint8ClampedArray.
 */
export const mix_channels_array = (inputs, width, height, config) => {
    if (inputs.length === 0) return null;

    const expected_length = width * height * 4;
    const num_pixels = width * height;
    const output = new Uint8ClampedArray(expected_length);
    const constants = config.constants;
    const terms = config.terms;

    for (let p = 0; p < num_pixels; p++) {
        const base_index = p << 2;

        for (let ch = 0; ch < 4; ch++) {
            let val = constants[ch] || 0;
            const channel_terms = terms[ch];

            if (channel_terms) {
                const term_count = channel_terms.length;
                for (let t = 0; t < term_count; t++) {
                    const term = channel_terms[t];
                    const input_data = inputs[term.input];
                    const input_index = base_index + term.channel;
                    val += term.coeff * input_data[input_index];
                }
            }
            
            output[base_index + ch] = fast_clamp(val);
        }
    }
