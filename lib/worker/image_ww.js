...
import { mix_channels_array, next_step_size } from "../module/image.js";
...
const init_promise = (async () => {
    try {
        ...
        const [image_resizer_vs, image_resizer_fs, chan_mix_vs, chan_mix_fs] = await Promise.all([
            fetch_shader('../shaders/image_resizer.vert'),
            fetch_shader('../shaders/image_resizer.frag'),

            fetch_shader('../shaders/channels_mixer.vert'),
            fetch_shader('../shaders/channels_mixer.frag'),
        ]);

        // Setup all shader programs in parallel
        const [image_resizer, channels_mixer] = await Promise.all([
            (() => {
            ...
            (() => {
                //console.log(chan_mix_vs, chan_mix_fs);
                const program = gpgpu.setup_program('channels_mixer', chan_mix_vs, chan_mix_fs, 'a_position');
                if (!program) throw new Error("Failed to setup program 'channels_mixer'");
                return program;
            })(),
        ]);
...
// Example usage function for channel mixing on GPU using GPGPU class
const mix_gpu = (gp, inputs_data, input_width, input_height, config) => {
    try {
        //console.log(`mix_gpu`, inputs_data, input_width, input_height, config);

        const program = gp.get_program('channels_mixer');
        program.use();

        const input_texes = [];
        for (let i = 0; i < 3; i++) {
            let data_view;
            if (i < inputs_data.length && inputs_data[i]) {
                data_view = new Uint8ClampedArray(inputs_data[i]);
            } else {
                data_view = new Uint8ClampedArray(input_width * input_height * 4);
            }
            const tex_name = `input_tex_${i}`;
            const tex = gp.create_texture(tex_name, i, data_view, input_width, input_height);
            input_texes.push(tex);
        }

        // Create output texture
        const output_tex_name = 'output_tex';
        gp.create_texture(output_tex_name, 3, null, input_width, input_height);

        // Setup framebuffer
        gp.setup_frame('main_frame', output_tex_name);

        // Set uniforms
        const gl = gp.gl;
        input_texes.forEach((tex, i) => {
            tex.activate();
            program.set(`tex${i}`, i);
        });

        // Prepare per-channel data
        const constants = config.constants.map(c => c / 255.0);
        for (let ch = 0; ch < 4; ch++) {
            program.set(`constant${ch}`, config.constants[ch]);
            const ch_terms = config.terms[ch] || [];
            program.set(`num_terms${ch}`, ch_terms.length);

            const coeffs = new Float32Array(4).fill(0);
            const input_idxs = new Int32Array(4).fill(0);
            const channels = new Int32Array(4).fill(0);

            ch_terms.forEach((term, t) => {
                coeffs[t] = term.coeff;
                input_idxs[t] = term.input;
                channels[t] = term.channel;
            });

            //console.log(`coeffs${ch}`, coeffs, `input_idxs${ch}`, input_idxs, `channels${ch}`, channels);
            gl.uniform1fv(program.uni_loc(`coeffs${ch}`), coeffs);
            gl.uniform1iv(program.uni_loc(`input_idxs${ch}`), input_idxs);
            gl.uniform1iv(program.uni_loc(`channels${ch}`), channels);
        }

        // Execute rendering
        gp.render(input_width, input_height);
        const result = gp.read_pixels(input_width, input_height);
        gp.clear_frame();
        //console.log(result);

        return result; // Uint8ClampedArray of RGBA pixels
    } catch (err) {
        gp.clear();
        console.error('GPU channel mix error:', err);
        throw err;
    }
};
...
const handle_channel_mixing = async (e) => {
    //console.log(`handle_channel_mixing`);
    const { inputs, width, height, config, id, extra } = e.data;
    //console.log(inputs, width, height, config, id, extra);

    try {
        //validate_image_data(src, 'src');
        const inputsTyped = inputs.map(input => new Uint8ClampedArray(input));
        let result = mix_channels_array(inputsTyped, width, height, config);
        //console.log(result);
        /*
        const has_webgl = !!gpgpu?.is_webgl_available;
        ...
        try {
            if (gpgpu?.is_webgl_available) {
                // Use GPU acceleration
                //throw new Error('Dummy error');
                const res = gpgpu.exec(mix_gpu, inputs, width, height, config);
                result = res.pixels;
            } else {
                throw new Error('WebGL not available');
            }
            ...
            console.warn('GPU resizer failed, falling back to CPU:', gpu_err.message);
            // Fallback to CPU implementation
            result = mix_channels_array(inputs, width, height, config)
        }
        ...
        */
        self.postMessage({
            type: 'mix', id,
            result: { data: result.buffer, width, height },
            extra,
        }, [result.buffer]);
